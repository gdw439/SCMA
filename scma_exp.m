...
FreWidth = 63;
n = 1:1:FreWidth;
nzc = exp( -1i*pi*34*(n-1).*n./FreWidth);                                         % ZC序列
Nzc = [nzc,nzc(1)];
NzcJ = conj(Nzc);
SynSeq = zeros(1,carrier);
...
for k = 1:length(SNR)
    ...
    while ((min(Nerr(:,k)) < maxNumErrs) && (Nbits(1,k) < maxNumBits))
        ...
        ofdm_oversamp = reshape(repmat(ofdm_transmit, oversamp, 1), 1, []); 
        
        ofdm_downsamp = ofdm_oversamp(1 : oversamp : end);
        
        [FrameLine,atp] = synchronize(ofdm_downsamp, SynPart, carrier+Interval, Interval,N+block_sum+1);
